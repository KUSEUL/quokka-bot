...
async def on_message(message):
    ...
    if msg.startswith("난 "):
        ...
        await smart_send(message, f"그러면은 알게쏘! {name} 기억할게용~ 🐾")
        return
...
# 음성채널 입장
    if ("들어와" in msg and message.author.voice):
        try:
            ...
            # 🔍 디버깅 로그
            print(f"🟢 유저 음성 채널 상태: {message.author.voice}")
            print(f"🟢 봇 voice_client 상태: {vc}")
            print(f"🟢 봇 현재 채널: {vc.channel if vc else '없음'}")

            if not vc or not vc.is_connected() or vc.channel != channel:
                await channel.connect()
                await smart_send(message, "쿼카 입자아앙~🐾")
            else:
                await smart_send(message, "이미 들어와있쪄용~😚")
        except Exception as e:
            import traceback
            traceback.print_exc()
            await smart_send(message, f"입장 실패했쪄용ㅠㅠ: {e}")
        return
    ...
    # 자동반응
    if any(kw in msg for kw in ["말해", "뭐해", "놀자", "왜", "코카야"]):
        messages = [
